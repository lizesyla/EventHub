  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSuccess(
          status === 'upcoming'
            ? "Event published successfully! It is now live."
            : "Event submitted successfully! It is awaiting admin approval."
        )
        setTitle('')
        setDescription('')
        setDate('')
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '32px' }}>
          <p style={{ color: colors.accent, fontSize: '12px', fontWeight: '700', letterSpacing: '3px', textTransform: 'uppercase', margin: '0 0 12px' }}>CREATE EVENT</p>
          <h2 style={{ fontSize: '28px', fontWeight: '800', color: colors.textMain, margin: '0 0 8px', letterSpacing: '-0.5px' }}>New Event</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} style={{
            width: '100%', padding: '14px', backgroundColor: loading ? '#4338ca' : colors.accent,
