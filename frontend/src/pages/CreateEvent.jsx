...
function CreateEvent() {
  ...
  const validateForm = () => {
    ...
    if (!date) {
      localErrors.date = "Date is required."
    }
    if (!location.trim()) {
    ...
    if (banner && !banner.type.startsWith("image/")) {
      localErrors.banner = "Only image files are allowed."
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const reviewStatus = data.review_status || data.event?.status || "pending"
        setSuccess(
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} style={{
            width: "100%",
            padding: "14px",
            backgroundColor: loading ? "#4338ca" : colors.accent,
            color: "#fff",
            border: "none",
            borderRadius: "10px",
            fontSize: "15px",
